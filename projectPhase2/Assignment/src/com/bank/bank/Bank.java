...
public class Bank {
  ...
  private static void atmInterface()
    ...
    while (selection != 5) {
      ...
      } else if (selection == 5) { // If choosing to exit program
        System.out.println("Exiting the ATM session. Good bye.");
      } else {
        System.out.println("Please enter a choice between 1 and 5 inclusive.");
      }
    }
  }
  ...
  private static void tellerInterface()
    ...
    while (selection != TELLER_OPTION_EXIT) {
      ...
      System.out.println(TELLER_OPTION_CUSTOMER_LEAVE + ". End current session.");
      ...
      if (selection == TELLER_OPTION_AUTHENTICATE_CUSTOMER) {
        ...
        User potentialCustomer = null;
        // default values for customerRoldId and customerIds
        int custRoleId = -1;
        int customerId = -1;
        // loop until we get a valid customer id
        do {
          System.out.println("Enter ID for the customer: ");
          customerId = InputFunctions.getAndCheckValidInt();
          custRoleId = DatabaseSelectHelper.getUserRole(userId);
          // check if it's a customer id and if it exists in db
          potentialCustomer = DatabaseSelectHelper.getUserDetails(customerId);
          if (potentialCustomer == null || !(potentialCustomer instanceof Customer)) {
            System.out.println("Invalid ID entered.\nPlease try again.");
          }
        } while (potentialCustomer == null || !(potentialCustomer instanceof Customer));
        
        if (DatabaseSelectHelper.getRole(custRoleId).equalsIgnoreCase(Roles.CUSTOMER.toString())) {
          throw new IllegalObjectTypeException("This is not a customer id.");
        }
        ...
        while (!c.authenticate(customerPassword)) {
          System.out.println("Error authenticating. Please try a different password.");
          customerPassword = br.readLine();
          c.authenticate(customerPassword);
        }
        // authenticate and set as current customer in tellerTerminal
        tellerTerminal.authenticateCustomer(customerPassword);
        System.out.println("Customer successfully authenticated.");
        // get a list of customer's accounts
        ...
        int custAge = InputFunctions.getAndCheckValidInt();
        ...
        try {
          ...
          System.out.println("Teller is not authenticated");
        } catch (IllegalInputParameterException e) {
          System.out.println("Some of the input parameters were incorrect");
        }
      ...
      } else if (selection == TELLER_OPTION_CHECK_BALANCE) { // If choosing to check balance
        // check if there's currently a customer
        if (tellerTerminal.currentCustomerPresent() == false) {
          continue;
        }
  ...
  private static void printAdminModeMenu() {
    ...
    System.out.println(ADMIN_OPTION_IMPORT_DB + ". Import the serialized database");
